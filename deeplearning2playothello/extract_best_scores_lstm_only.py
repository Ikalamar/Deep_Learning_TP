...
from networks_2405536 import LSTMs
...
def build_dev_loader(batch_size=1000, len_samples=5, path_dataset="../dataset/"):
    """
    Build dev loader for Many2One (LSTM).
    """
    dataset_conf = {
        "filelist": "dev.txt",
...
def main():
    ...
    # Scan only save_models_LSTM* directories
    all_results = {}
    colors = {}
    color_list = ['blue', 'red', 'green', 'orange', 'purple', 'brown', 'pink', 'gray', 'cyan', 'magenta']
    marker_list = ['o', 's', '^', 'v', 'D', 'p', '*', 'h', 'X', '+']
    
    lstm_dirs = [item for item in sorted(os.listdir('.')) 
                 if item.startswith('save_models_') and 'LSTM' in item and os.path.isdir(item)]
    
    for i, item in enumerate(lstm_dirs):
        try:
    ...
    log_file = os.path.join(logs_dir, f'best_scores_lstm_{now_filename}.log')
    with open(log_file, 'w', encoding='utf-8') as f:
        f.writelines(out_lines)

    print(f'Wrote {log_file}')

    # Generate graph with LSTM models only
    ...
    plt.title('LSTM Models - Accuracy vs Epoch (Dev Set)', fontsize=14, fontweight='bold')
    plt.legend(fontsize=10, loc='best')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    graph_file = os.path.join(logs_dir, f'accuracy_curves_lstm_{now_filename}.png')
